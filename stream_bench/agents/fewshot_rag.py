...
class FewShotRAGAgent(Agent):
    ...
    def __init__(self, config: dict) -> None:
        ...
        elif (config["agent_name"] == Method.PAIR_SHOTS_ICL.value):
            assert config.get("mode", "normal") == "normal", \
                "pair_shots_icl requires mode == normal"
            self.mode = "normal"
        else:
            self.mode = "normal"
    ...
    def __call__(
        ...
        **kwargs
        ...
        if self.method == Method.PAIR_SHOTS_ICL.value:
            # pair_shots_icl: pair up correct and incorrect cases one by one
            num_pairs = min(len(y_shots), len(n_shots))
            paired_shots = []
        ...
        if self.method == Method.CORRECT_SELF_COT.value or self.enabled_cot:
        ...
        if len(shots):
            ...
            prompt = template_wo_rag
        # Inference
        pred_text_raw, pred_info = self.llm(prompt=prompt, max_tokens=self.llm_config["max_tokens"], temperature=self.llm_config["temperature"])

        # Filter pred_text based on conditions (similar to policy.py)
    ...
    def update(self, has_feedback: bool, **feedbacks) -> bool:
        ...
        elif self.method in {Method.MEM_PROMPT.value, Method.PAIR_SHOTS_ICL.value}:
            assert ("self_output" in feedbacks) and ("is_correct" in feedbacks)
            answer = feedbacks["self_output"]
        ...
        if self.method in {Method.MEM_PROMPT.value, Method.PAIR_SHOTS_ICL.value}:
