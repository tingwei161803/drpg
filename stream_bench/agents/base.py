...
class Agent:
    LOG_KEYS = [
        ...
        "num_thinking_tokens",  # the total number of thinking tokens in __call__()
    ]
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.llm = get_llm(series=self.llm_config["series"],
                          model_name=self.llm_config["model_name"])

        # Initialize second LLM if specified
        if "llm2" in config:
            self.llm2 = get_llm(series=config["llm2"]["series"],
                               model_name=config["llm2"]["model_name"])
            self.llm2_config = config["llm2"]
        else:
            self.llm2 = None
            self.llm2_config = None

        # Initialize critic LLM if specified
        if "critic_llm" in config:
            self.critic_llm = get_llm(
                series=config["critic_llm"]["series"],
                model_name=config["critic_llm"]["model_name"]
            )
            self.critic_llm_config = config["critic_llm"]
        else:
            self.critic_llm = None
            self.critic_llm_config = None

        if "policy_llm" in config:
            self.policy_llm = get_llm(
                series=config["policy_llm"]["series"],
                model_name=config["policy_llm"]["model_name"]
            )
            self.policy_llm_config = config["policy_llm"]
        else:
            self.policy_llm = None

        # Initialize second policy LLM if specified
        if "policy_llm2" in config:
            self.policy_llm2 = get_llm(
                series=config["policy_llm2"]["series"],
                model_name=config["policy_llm2"]["model_name"]
            )
            self.policy_llm2_config = config["policy_llm2"]
        else:
            self.policy_llm2 = None
