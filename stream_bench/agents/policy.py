...
class PolicyAgent(Agent):
    ...
    def generate_rag_policy(self, y_shots: list[str], n_shots: list[str], **kwargs) -> tuple[str, dict, str, str]:
        ...
        Returns:
            tuple[str, dict, str, str]: Generated policy, token information, policy prompt, policy raw output
        """
        ...
        template_name = (
            ...
            f"with_{self.policy_generator_mode}_cases_rule_ver{'1' if self.prompt_ver == 'original' else 'LEAP'}"
        )
    ...
    def generate_posthoc_policy(self, y_shots: list[str], **kwargs) -> tuple[str, dict, str, str]:
        ...
        if self.last_case_correctness:
            return self.current_policy, {"num_input_tokens": 0, "num_output_tokens": 0}, "", ""
        ...
        **kwargs
        ...
        if self.rag.insert_acc > 0:
            if self.mode == "normal":
                ...
                # If similar_at_bottom, reverse the order of shots
    ...
    def update(self, has_feedback: bool, **feedbacks) -> bool:
        ...
        if self.method in {Method.POSTHOC_POLICY.value, Method.RAG_POLICY.value}:
            ...
            if feedbacks["is_correct"]:
                corr_verb = "correct"
                self.confidence_score = round(self.ewma_alpha * 1 + (1 - self.ewma_alpha) * self.confidence_score, 2)
                self.last_case_correctness = True
            else:
                corr_verb = "not correct"
                self.confidence_score = round(self.ewma_alpha * 0 + (1 - self.ewma_alpha) * self.confidence_score, 2)
