...
class GeminiDev:
    ...
    def __call__(self, prompt: str, max_tokens=512, temperature=0.0, top_p=1, top_k=1) -> tuple[str, dict]:
        ...
        if res.text:
            ...
            raise requests.exceptions.HTTPError("API return nothing")
        return res.text, res_info


if __name__ == "__main__":
    # llm = GeminiDev(model_name="gemma-3-27b-it")
    # llm = GeminiDev(model_name="gemini-2.5-flash-preview-05-20")
    llm = GeminiDev(model_name="gemini-2.0-flash-lite-001")
    res_text, res_info = llm("Hello! Can you breifly introduce yourself?")
    print(res_text)
    print(res_info)